...
| `tools/build-kit.mjs` | THE EMITTER. Turns a brand pack into that brand's whole stylesheet: `<slug>-kit.css`, `fonts.html`, `tokens.generated.css`. Brand-blind — no hex, no font name, no slug in the source; it reads roles and asks the pack. `bbe kit` on the CLI. `--verify` proves the output against a reference kit on two levels, a normalized text diff and computed styles in a real browser. `--scheme light|dark|system` picks the colour scheme (ruling 66); the light emission is always what the reproduction proof measures. See `docs/EMITTER.md`. |
| `tools/kit-verify-computed.mjs` | Level 2 of that verify. Builds a harness page carrying every component module, loads it under both stylesheets, and compares `getComputedStyle` property by property. Proves the brand face actually loaded before it believes a single number. On a non-light scheme it also drives every combination of OS preference and reader choice, checks the remapped tokens flip and the ladder holds, and audits WCAG AA on the dark render. |
