...
| `buildDir` | no | Where the build writes pages (`"dist"`). Set it on any site whose pages are generated by a build script, like every engine site. The kit link check and the head check then read the built pages, and the reusable workflow builds before gating. Without it, a generated site gives those checks nothing to read. |
